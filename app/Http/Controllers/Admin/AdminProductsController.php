...
class AdminProductsController extends Controller
{
	...
	public function products(Request $request){
		...
		$data = DB::table('products_to_categories')
			...
			if(isset($_REQUEST['categories_id']) and !empty($_REQUEST['categories_id'])){
				...
				if(isset($_REQUEST['product']->serial) and !empty($_REQUEST['product']->serial)){
					$data->where('products_name', 'like', '%' . $_REQUEST['product']->serial . '%');
				}
		...
		return view("admin.products",$title)->with('results', $results)->with('cart_data',$cart_data);
	}
	...
	public function addproduct(Request $request){
		...
		$language_id      =   '1';
		$key = Keygen::numeric(6)->generate();
		$pref = Config::get('invent_prefix', 'FBN');
		$result = array();
		...
		return view("admin.addproduct", $title, compact('pref', 'key', 'result'))->with('cart_data',$cart_data);
	}
	...
	public function addproductattribute(Request $request){
		...
		return view("admin.addproductattribute", $title)->with('result', $result)->with('cart_data',$cart_data);
	}
	...
	public function addproductimages(Request $request){
		...
		return view("admin.addproductimages", $title)->with('result', $result)->with('cart_data',$cart_data);
	}
	...
	{
		$title = array('pageTitle' => Lang::get("labels.ImportNewProducts"));
		return view('admin.importproducts', $title);
	}
        ...
        if($request->hasFile('sample_file')){
            ...
            if($data->count()){
                ...
                foreach ($data as $key => $value) {
					...
					if (Product::where('serial', '=', $value->serial)->exists()) {
						$key = Keygen::numeric(6)->generate();
						$serial = env('INVENT_PREFIX');
						$newkey = $serial . ' ' . $key;
					 }
					 else{
						$newkey = $value->serial;
					...
					$arr[] = [
							  ...
							  'products_slug' => $value->products_slug, 
							  'products_status' => $value->products_status, 
							  'is_feature' => $value->is_feature, 
							  'low_limit' => $value->low_limit, 
							  'products_date_added' => $value->products_date_added, 
							  'products_date_available' => $value->products_date_available, 
							  'products_tax_class_id' => $value->products_tax_class_id];
                ...
                if(!empty($arr)){
                    ...
                    DB::table('products')->insert($arr,$newkey);
			...
			DB::table('products_to_categories')->insert([
				'id'   	=>     $id,
				'categories_id'     =>     ProductCategory::CATEGORY
			]);
			

        }
		\Session::flash('alert-success', 'The Products Excel Sheet has been Imported successfully!');
	...
	public function editproduct(Request $request){
		...
		return view("admin.editproduct", $title, compact('pref', 'key'))->with('result', $result)->with('cart_data',$cart_data);
	}
	...
	public function editproductattribute(Request $request){
		...
		return view("admin/editproductattributeform")->with('result', $result)->with('cart_data',$cart_data);
	}
	...
	public function editdefaultattribute(Request $request){
		...
		return view("admin/editdefaultattributeform")->with('result', $result)->with('cart_data',$cart_data);
	}
	...
	public function deleteproductattributemodal(Request $request){
		...
		return view("admin/deleteproductattributemodal")->with('result', $result)->with('cart_data',$cart_data);
	}
	...
	public function editproductimage(Request $request){
		...
		return view("admin/editproductimageform")->with('result', $products_images)->with('cart_data',$cart_data);
	}
	...
	public function attributes(Request $request){
		...
		return view("admin.attributes",$title)->with('result', $result)->with('cart_data',$cart_data);
	}
	...
	public function addattributes(Request $request){
		...
		return view("admin.addattributes",$title)->with('resutls', $resutls)->with('cart_data',$cart_data);
	}
	...
	public function editattributes(Request $request){
		...
		return view("admin.editattributes",$title)->with('attributes', $attributes)->with('cart_data',$cart_data);
	}
	...
	public function addattributevalue(Request $request){
		...
		return view("admin.attributesTable")->with('attributes', $attributes)->with('cart_data',$cart_data);
	}
